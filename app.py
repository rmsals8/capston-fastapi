...
# 위치 정보 보강 함수 (원본 함수 유지)
def enhance_location_data(schedule_data):
    """
    Google Places API를 사용하여 일정 데이터의 위치 정보를 보강합니다.
    """
    print("위치 정보 보강 시작...")
            ...
            if search_term:
                ...
                if place_info:
                    ...
                    if place_info.get("name"):
                        print(f"장소명 업데이트: '{schedule.get('name', '')}' -> '{place_info['name']}'")
                        schedule["name"] = place_info["name"]
...
# Spring 백엔드로 최적화 요청을 보내는 함수
def optimize_schedule(schedule_data):
    """
    Spring 백엔드의 /api/v1/schedules/optimize-1 엔드포인트로 
    일정 최적화 요청을 전송합니다.
    """
    spring_backend_url = "http://localhost:8080/api/v1/schedules/optimize-1"
    
    try:
        # ScheduleOptimizationRequest 형식에 맞게 데이터 구성
        ...
        # 요청 보내기 전 로깅
        print(f"Spring 백엔드로 전송할 최적화 요청: {json.dumps(optimization_request, ensure_ascii=False)}")
        ...
        print(f"Spring 백엔드 통신 오류: {str(e)}")
        if hasattr(e, 'response') and e.response is not None:
            print(f"응답 내용: {e.response.text}")
        raise Exception(f"일정 최적화 중 오류 발생: {str(e)}")
...
@app.post("/enhance-schedule")
async def enhance_schedule(request: ScheduleRequest):
    """
    음성 입력에서 일정을 추출하고 위치 정보를 보강합니다.
    """
    try:
        print(f"음성 입력 받음 (위치 정보 보강): '{request.voice_input}'")
        ...
        print(f"위치 정보 보강된 일정 데이터: {json.dumps(enhanced_data, ensure_ascii=False)}")
...
# 새로운 엔드포인트 추가 - 향상된 위치 정보 보강 + 최적화 기능
@app.post("/enhanced-optimize-schedule")
async def enhanced_optimize_schedule(request: ScheduleRequest):
    """
    음성 입력에서 일정을 추출하고, 향상된 방식으로 위치 정보를 보강한 후 
    Spring 백엔드에 최적화 요청을 보냅니다.
    """
    try:
        print(f"음성 입력 받음 (향상된 최적화): '{request.voice_input}'")
        ...
        # 5. Spring 백엔드로 최적화 요청 전송
        print("Spring 백엔드로 최적화 요청 전송 중...")
        optimized_schedule = optimize_schedule(enhanced_data)
        print("최적화 요청 완료")
        
        # 6. 최적화된 일정 반환
        return {
            "extracted_schedule": schedule_data,
            "enhanced_schedule": enhanced_data,
